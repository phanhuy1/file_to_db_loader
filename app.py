...
def get_column_names(schemas, ds_name, sorting_key='column_position'):
    return list(map(lambda x: x.get('column_name'), sorted(schemas.get(ds_name), key=lambda x: x.get('column_position'))))
...
def db_loader(src_base_dir, db_conn_uri, ds_name):
    ...
    if len(files) == 0:
        raise NameError(f'No files found for {ds_name} in {src_base_dir}')
    for file in files:
...
def process_files(ds_names=None):
    ...
    processors = len(ds_names) if len(ds_names) < 3 else 3
